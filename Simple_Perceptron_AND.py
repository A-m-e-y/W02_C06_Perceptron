...
def generate_nand_data(n_inputs):
    """Generate training data for an N-input AND gate."""
    X = np.array([list(map(int, f"{i:0{n_inputs}b}")) for i in range(2**n_inputs)])
    y = np.array([1 if np.sum(x) == n_inputs else 0 for x in X])  # AND logic
    return X, y

def main():
    if len(sys.argv) != 2:
        print("Usage: python3 Simple_Perceptron_AND.py <number_of_inputs>")
        sys.exit(1)
    ...
    # Generate training data for N-input AND gate
    ...
    # Train perceptron
    print(f"Training the perceptron for {n_inputs}-input AND logic:")
    p.train(X, y)
