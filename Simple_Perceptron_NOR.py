...
def generate_nand_data(n_inputs):
    """Generate training data for an N-input NOR gate."""
    X = np.array([list(map(int, f"{i:0{n_inputs}b}")) for i in range(2**n_inputs)])
    y = np.array([1 if np.sum(x) == 0 else 0 for x in X])  # NOR logic
    return X, y

def main():
    if len(sys.argv) != 2:
        print("Usage: python3 Simple_Perceptron_NOR.py <number_of_inputs>")
        sys.exit(1)
    ...
    # Generate training data for N-input NOR gate
    ...
    # Train perceptron
    print(f"Training the perceptron for {n_inputs}-input NOR logic:")
    p.train(X, y)
