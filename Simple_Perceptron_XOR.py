...
def generate_nand_data(n_inputs):
    """Generate training data for an N-input XOR gate."""
    X = np.array([list(map(int, f"{i:0{n_inputs}b}")) for i in range(2**n_inputs)])
    y = np.array([1 if np.sum(x) % 2 == 1 else 0 for x in X])  # XOR logic
    return X, y

def main():
    if len(sys.argv) != 2:
        print("Usage: python3 Simple_Perceptron_XOR.py <number_of_inputs>")
        sys.exit(1)
    ...
    # Generate training data for N-input XOR gate
    ...
    # Train perceptron
    print(f"Training the perceptron for {n_inputs}-input XOR logic:")
    p.train(X, y)
